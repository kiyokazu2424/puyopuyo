class Player {
  ...
  static createNewPuyo () {
      ...
      if(!this.nextPuyo) {
        ...
        this.nextNextCenterPuyo = Math.floor(Math.random() * puyoColors) + 1;
        this.nextNextMovablePuyo = Math.floor(Math.random() * puyoColors) + 1;
        this.nextNextCenterPuyoElement = PuyoImage.getPuyo(this.nextNextCenterPuyo);
        this.nextNextMovablePuyoElement = PuyoImage.getPuyo(this.nextNextMovablePuyo);

        this.nextPuyo = true
        console.log("初期の作成完了")
      }

      // ぷよの順番をずらしてネクネクを補充（順番が大事）
      // ネクスト→現在のぷよの移動
      ...
      console.log("ネクスト移動")
      // ネクネク→ネクストの移動
      this.nextCenterPuyo = this.nextNextCenterPuyo
      this.nextMovablePuyo = this.nextNextMovablePuyo
      this.nextCenterPuyoElement = this.nextNextCenterPuyoElement
      this.nextMovablePuyoElement = this.nextNextMovablePuyoElement
      console.log("ネクネク移動")
      // ネクネクの補充
      this.nextNextCenterPuyo = Math.floor(Math.random() * puyoColors) + 1;
      this.nextNextMovablePuyo = Math.floor(Math.random() * puyoColors) + 1;
